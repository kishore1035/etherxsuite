 ...
 */
export function saveSheetData(sheetId: string, data: { cells: Record<string, any>, images?: any[], shapes?: any[], charts?: any[] }, hash?: string): void {
  const keys = getSheetStorageKeys(sheetId);
  const timestamp = new Date().toISOString();

  try {
    // Write data to sheet-specific key
    safeSetItem(keys.data, JSON.stringify(data));
    safeSetItem(keys.timestamp, timestamp);

    if (hash) {
      safeSetItem(keys.hash, hash);
    }

    console.log(`✅ Saved sheet ${sheetId} data (${Object.keys(data.cells || {}).length} cells, ${data.images?.length || 0} images, ${data.shapes?.length || 0} shapes)`);
  } catch (error) {
...
export function initializeBlankSheet(sheetId: string): void {
  ...
  try {
    ...
    safeSetItem(keys.data, JSON.stringify({}));
    safeSetItem(keys.timestamp, timestamp);
 ...
 */
export function loadSheetData(sheetId: string): { cells: Record<string, any>, images?: any[], shapes?: any[], charts?: any[] } | null {
  const keys = getSheetStorageKeys(sheetId);
  ...
  try {
    ...
    console.log(`✅ Loaded sheet ${sheetId} data from ${timestamp}`);
    return data;
  } catch (error) {
...
export function debouncedSaveSheetData(
  ...
  const timeoutId = setTimeout(() => {
    saveSheetData(sheetId, data, hash);
